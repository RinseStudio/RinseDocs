...
export function buildNavigation(collectionPages, currentPath = '', overrides = []) {
  ...
  function groupByGroup(input) {
    const grouped = input.reduce((accumulator, currentItem) => {
      ...
      if (!accumulator[currentItem.group]) {
        accumulator[currentItem.group] = { label: currentItem.group, pages: [] };
      }
  ...
  function reorderArrayBasedOnOverrides(sampleArray, overridesArray) {
    const overrideMap = new Map(overridesArray.map((item) => [item.label[0], item.label[1]]));
    ...
    sampleArray.sort((a, b) => {
      ...
      if (labelA !== undefined && labelB !== undefined) {
        return overridesArray.findIndex((item) => item.label[0] === a.label) - overridesArray.findIndex((item) => item.label[0] === b.label);
  ...
  function updateLabels(sampleArray, overridesArray) {
    ...
    const overridesMap = new Map(overridesArray.map(({ label }) => [label[0], label[1]]));
